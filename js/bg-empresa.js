(function () {
  ...
  var lastSpark = 0;
  var C = { bg:'#f5f6f8', grid:'rgba(150,162,180,0.14)', gridAcc:'rgba(120,138,158,0.24)', gear:'rgba(150,160,175,0.14)', gearStroke:'rgba(120,138,158,0.28)' };
  ...
  function drawCircuits(t){for(var ci=0;ci<CIRCUITS.length;ci++){var c=CIRCUITS[ci],alpha=0.06+0.06*Math.sin(t*c.speed+c.pulse),off=-scrollY*0.12;ctx.save();ctx.translate(0,off);ctx.strokeStyle='rgba(4,84,115,'+alpha+')';ctx.lineWidth=1;ctx.beginPath();for(var si=0;si<c.segs.length;si++){if(si===0)ctx.moveTo(c.segs[si].x,c.segs[si].y);else ctx.lineTo(c.segs[si].x,c.segs[si].y);}ctx.stroke();for(var si2=0;si2<c.segs.length;si2++){ctx.beginPath();ctx.arc(c.segs[si2].x,c.segs[si2].y,2.5,0,Math.PI*2);ctx.fillStyle='rgba(4,84,115,'+(alpha*2)+')';ctx.fill();}ctx.restore();}}
